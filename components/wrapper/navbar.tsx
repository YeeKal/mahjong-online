...
export default function Navigation() {
  ...
  return (
    <header className="min-w-full  p-2 border-b z-10 bg-background text-foreground">
      <div className="px-8 sm:px-10 h-10 flex items-center justify-between">

        <Link href="/" className="flex items-center gap-2" prefetch={false}>
          <MountainIcon className="w-8 h-8" />
          <SeparatorIcon/>
          <span className="text-xl font-semibold">Cogito.AI</span>
        </Link>

        <nav className="hidden sm:flex  items-center justify-between gap-4 md:gap-10 lg:gap-20">
            {
                navText.map((item, index) => (
                    <Link key={index} href={item.href} className="text-xl text-foreground   hover:underline underline-offset-4 hover:decoration-theme" prefetch={false}>
                        {item.text}
                    </Link>
                ))
            }
